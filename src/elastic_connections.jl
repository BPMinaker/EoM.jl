...
if(s>0)  ## If the deflection matrix has more than zero rows (i.e. there are elastic items in the system)
	...
	for i in the_system.flex_points  ## For each elastic point item
		...
		flex_point_stiff[idx:idxe,idx:idxe]+=spdiagm(0=>[i.stiffness[1]*[1;1;1];i.stiffness[2]*[1;1;1]])
		flex_point_dmpng[idx:idxe,idx:idxe]+=spdiagm(0=>[i.damping[1]*[1;1;1];i.damping[2]*[1;1;1]])
