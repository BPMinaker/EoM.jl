function decompose(ss_eqns::EoM.ss_data, args...)
    ...
    ## any later version.
    ##
    ## decompose.jl is distributed in the hope that it will be useful, but
    ...
    A = round.(ss_eqns.A, digits = 10)
    B = round.(ss_eqns.B, digits = 10)
    C = round.(ss_eqns.C, digits = 10)
    D = round.(ss_eqns.D, digits = 10)
...
function group_vals(val, vect)
    ...
    p = sortperm(
        round.(val, digits = 5),
        by = x -> (isreal(x), real(x) > 0, abs(x), real(x), abs(imag(x)), -imag(x)),
    )
    val = val[p]
    #	display(val)
    vect = vect[:, p]
...
function check_vecs!(val, vect, A; left = false, verbose = true)
    ...
    if d > 0
        verbose && println(
            "Rank ",
            r,
            ", eigenvectors are defective.  Attempting minimization anyway...",
        )
        idx = ones(n) .== 1
        idx[2] = false
        i = 2
