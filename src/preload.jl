...
ind_test_mtx=[zeros(q,q) data.constraint zeros(q,p); data.constraint' data.stiffness data.deflection'*data.selection'*diagm(data.subset_spring_stiffness); zeros(p,q) diagm(data.subset_spring_stiffness)*data.selection*data.deflection diagm(data.subset_spring_stiffness)]
...
if(s>0)
	...
	if(rk==s)  ## If rank = number of cols -> statically determinate [full rank]
		...
		if(rk==t)	## Elastically determinate
			...
			if(verb)
				...
				println("Attempting a trial solution anyway, checking whether the system is in equilibrium...")
			end
			sumf=test_mtx[1:r,:]*lambda+data.force  ## test matrix*lambda+f <- should be zero
			temp=(sumf'*sumf)[1]
			if(temp<1e-5*length(sumf))  ## If magnitude squared of sumf < small  i.e., equals ~zero
				verb && println("System is in equilibrium. Good.")
			else
	...
	lambda=Vector{Float64}(0) ## If there are no constraints or elastic items (odd, but possible)
	sumf=data.force
	temp=(sumf'*sumf)[1]
	if(temp<1e5 *length(sumf))  ## If magnitude squared of sumf < small  i.e., equals ~zero
		verb && println("System is in equilibrium. Good.")
	else
		println("The squared force error is $temp.")
		error("System is not in equilibrium.")  ## Equilibrium cannot be found, thus the system cannot be analyzed
