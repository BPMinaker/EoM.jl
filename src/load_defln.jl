function load_defln(the_system, dir_output)
    ...
    for item in [the_system.rigid_points; the_system.flex_points]
        frc = item.b_mtx[1]' * item.preload[1:item.forces]
        mmt = item.b_mtx[2]' * item.preload[item.forces+1:end]
    ...
    for item in the_system.beams
        ...
        D = [
            ...
            0 0 -1 0 0 0 1 0
            0 2/l -1 0 0 -2/l -1 0
        ]  ## Relate the beam stiffness matrix to the deflection of the ends (diagonalize the typical beam stiffness matrix!)
        temp =
            diagm(0 => (D' * item.preload)) * [
                item.b_mtx[1] zeros(item.forces, 3)
                zeros(item.moments, 3) item.b_mtx[2]
                item.b_mtx[1] zeros(item.forces, 3)
                zeros(item.moments, 3) item.b_mtx[2]
            ]
    ...
    for item in the_system.bodys[1:end-1]
        ...
        println(
            defln_f,
            "{ } { } rotation ",
            item.angular_deflection[1],
            " ",
            item.angular_deflection[2],
            " ",
            item.angular_deflection[3],
        )
        idx += 1
    end
