...
println(stiff_f,"###### Connection Data\nnum name stiffness damping t_stiffness t_damping")

## Body data
idx=1
for item in the_system.bodys[1:end-1]
	print(body_f,"{",idx,"} {",item.name,"} ",item.mass," ",item.location[1]," ",item.location[2]," ",item.location[3]," ")
	print(body_f,item.moments_of_inertia[1]," ",item.moments_of_inertia[2]," ",item.moments_of_inertia[3]," ")
	println(body_f,item.products_of_inertia[1]," ",item.products_of_inertia[2]," ",item.products_of_inertia[3])
	idx+=1
end
close(body_f)
...
for item in the_system.rigid_points
	print(point_f,"{",idx,"} {",item.name,"} ",item.location[1]," ",item.location[2]," ",item.location[3]," ")
	if norm(item.axis)>0
...
for item in the_system.flex_points
	...
	println(stiff_f,"{",idx2,"} {",item.name,"} ",item.stiffness[1]," ",item.damping[1]," ",item.stiffness[2]," ",item.damping[2])
	idx+=1
	idx2+=1
end
