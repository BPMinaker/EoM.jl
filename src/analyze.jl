...
# lowest low eigenvalue, even number in Hz
(low < -2) && (low=-2) # limit the min 0.01 Hz
high=ceil(log10(2.0*maximum(upper)/2/pi))
# highest high eigenvalue, even number in Hz
...
for i=1:nvpts
	...
	# compute frequency response, one I/O pair at a time
	for j=1:wpts
	...
	try
		...
		# for m=1:nin
		# 	for n=1:nout
		# 		temp=dss_data(dss_eqns[i].A,dss_eqns[i].B[:,m:m],dss_eqns[i].C[n:n,:], dss_eqns[i].D[n:n,m:m],dss_eqns[i].E,p)
		# 		ss_eqns,_=dss2ss(temp)
		# 		try
